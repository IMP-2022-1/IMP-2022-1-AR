...
public class PlanePotion : MonoBehaviour
{
    ...
    private void spawnUnRealPotion(ARPlane plane)
    {
        ...
        {
            Debug.Log("unrealPotion Spawn!!!");
            Vector3 potionPosition = Random.insideUnitCircle.normalized;
            unrealPotion = Instantiate(unrealPotionToInstantiate, plane.center, Quaternion.identity);
            potionCheck = false;
        }*/
    }
    ...
    private void spawnRealPotion()
    {
        ...
        /*
        Vector3 potionPosition = Random.insideUnitCircle.normalized;
        potion = Instantiate(potionToInstantiate, new Vector3(potionPosition.x, 0, potionPosition.z), Quaternion.identity);
        */


        // Spawn in plane's center
        Vector3 unrealPotionPosition = unrealPotion.transform.position;
        if (Vector3.Distance(ARCameraPosition, unrealPotionPosition)<0.2f)
        {
